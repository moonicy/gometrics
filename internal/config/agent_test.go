...
func TestNewAgentConfig_Flags(t *testing.T) {
	...
	if ac.CryptoKey != "testcryptokey" {
		t.Errorf("Expected CryptoKey to be %s, got %s", "testcryptokey", ac.CryptoKey)
	}
}
...
func TestNewAgentConfig_EnvVars(t *testing.T) {
	...
	if ac.CryptoKey != "envcryptokey" {
		t.Errorf("Expected CryptoKey to be %s, got %s", "envcryptokey", ac.CryptoKey)
	}
}
