...
func TestNewServerConfig_WithEnvVars(t *testing.T) {
	...
	if sc.CryptoKey != "envcryptokey" {
		t.Errorf("Expected CryptoKey to be envcryptokey, got %s", sc.CryptoKey)
	}
}
...
func TestNewServerConfig_FlagsAndEnvVars(t *testing.T) {
	...
	if sc.CryptoKey != "" {
		t.Errorf("Expected CryptoKey to be empty, got %s", sc.CryptoKey)
	}
}
